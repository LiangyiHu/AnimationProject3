...
const GLfloat ENERGY_RESERVED_RATE = 0.8f;
...
public:
	...
	glm::vec3 currentPosition,speed,acceleration, offsetThisTime, gravity=glm::vec3(0.0f, -98.0f, 0.0f), force, currentRotationAxle,deltaPosition;
	glm::vec4 currentQuat;
	GLfloat lastTimeUpdated, deltaTime, mass, angularSpeed, deltaAngle;
	GLuint objectID;
	bool gravitational;
	float radius,scale;
	enum collisionDetectType
	{
		groundDetect, sphereDetect
	};

	Model(GLchar* path, GLuint objectID, float radius, GLfloat mass, bool gravitational=true,  glm::vec3 initialPosition=glm::vec3(0.0f, 0.0f, 0.0f),glm::vec3 initialSpeed=glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3 initialForce=glm::vec3(0.0f, 0.0f, 0.0f))
	{
		this->mass = mass;
		this->radius = radius;
		...
		this->acceleration = gravitational?(gravity+force/mass): (force / mass);
		this->currentRotationAxle = glm::vec3(0.0f,0.0f,-1.0f);
		this->angularSpeed = 0.0f;
		this->currentQuat = getQuaternionFromAngleAndAxle(0.0f, currentRotationAxle);
		this->currentTransformationMatrix = glm::translate(glm::mat4(), this->currentPosition);

		this->currentTransformationMatrix = glm::scale(currentTransformationMatrix, glm::vec3(scale, scale, scale));
	...
	void updateMotion() {
		...
		currentQuat = combineQuatRotations(currentQuat,getQuaternionFromAngleAndAxle(deltaAngle,currentRotationAxle));
		
		currentTransformationMatrix = glm::translate(glm::mat4(), currentPosition);
		currentTransformationMatrix = glm::scale(currentTransformationMatrix, glm::vec3(scale, scale, scale));
		currentTransformationMatrix = glm::rotate(currentTransformationMatrix, getAngleFromQuat(currentQuat),getAxleFromQuat(currentQuat));
	...
	static void collisionDetect(Model* x, Model* y, collisionDetectType detectMethod) {
		if (detectMethod == groundDetect) {
			...
			std::cout << b << std::endl;
			std::cout << c << std::endl;
			std::cout << (-b - glm::sqrt(glm::pow(b, 2) - 4 * a*c)) / (2 * a) << std::endl;
			std::cout << (-b + glm::sqrt(glm::pow(b, 2) - 4 * a*c)) / (2 * a) << std::endl;
			std::cout << optimizedP << std::endl;*/
			x->speed = x->speed - optimizedP/x->mass*n;
			y->speed = y->speed + optimizedP/y->mass*n;
			if (y->objectID == 2) {
				printThisVector(y->speed);
			}
			//reshape speed if too small
			//std::cout << "a collision of " << x->objectID << "  and " << y->objectID << std::endl;
			...
			GLfloat a1 = glm::dot(x->speed, n);
			GLfloat a2 = glm::dot(y->speed, n);
			GLfloat optimizedP = 2.0*(a1 - a2)*(x->mass * y->mass) / (x->mass + y->mass);
			std::cout << 2 * (a2-a1);
			std::cout << optimizedP << std::endl;
			x->speed = x->speed - optimizedP/x->mass*n;
			y->speed = y->speed + optimizedP/y->mass*n;
